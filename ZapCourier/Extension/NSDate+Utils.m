...
static const unsigned componentFlags = (NSCalendarUnitYear| NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitWeekday |  NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond | NSCalendarUnitWeekOfMonth | NSCalendarUnitWeekdayOrdinal | NSCalendarUnitWeekOfMonth | NSCalendarUnitWeekOfYear);
...
//        NSInteger t = 28800 - [[NSTimeZone systemTimeZone] secondsFromGMT];
        NSString *dt = [formatter stringFromDate:self];
        return dt;
...
+ (NSDate *)getCurrentUTCDate
{
    ...
    //    DLog(@"%@",[NSDate dateFromString:dateString]);
    return [self dateFromString:dateString withFormat:@"yyyy-MM-dd HH:mm:ss"];
}
+(NSString*)convertToTimeAgoWithTime:(double)time{
    NSDate *dateCompare = [NSDate dateWithTimeIntervalSince1970:time];
    NSTimeInterval now = [[NSDate date] timeIntervalSince1970];
    double deltaSeconds = fabs(now - time);
    double deltaMinutes = deltaSeconds / 60.0f;
    
//    Listed a minute ago by xxx
//    Listed %s minutes ago by xxx
//    Listed an hour ago by xxx
//    Listed %s hours ago by xxx
//    Listed on yesterday by xxx
//    Listed on dd MMMM yyyy by xxx (ex: Listed on 28 September 2015 by xxx)
    
    if (dateCompare.isToday) {
        if(deltaSeconds < 120) {
    ...
    }else if (dateCompare.isYesterday){
        return @"on yesterday";
    }else{
        return [NSString stringWithFormat:@"on %@",[dateCompare stringWithFormat:@"dd MMMM yyyy"]];
    }
...
+ (long long) getTodayTimeSeconds:(NSString *)outputTimeZoneString :(NSString *)outputDateFormat {
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setTimeZone:[NSTimeZone timeZoneWithAbbreviation:outputTimeZoneString]];
    [dateFormatter setDateFormat:outputDateFormat];
    NSString    *strTime = [dateFormatter stringFromDate:[NSDate date]];
    
    NSDate *objDate  = [dateFormatter dateFromString:strTime];
    long long timeInSeconds = (long long)[objDate timeIntervalSince1970]*1.0;
    
    return timeInSeconds;
}

//Convert Time String input to seconds
+(long long) getTimeLongFromTimeString: (NSString *) dateString :(NSString *)currTimeZoneString :(NSString *)outputDateFormat {
    NSDate *dateFromString = [[NSDate alloc] init];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setTimeZone:[NSTimeZone timeZoneWithAbbreviation:currTimeZoneString]];
    [dateFormatter setDateFormat:outputDateFormat];
    dateFromString = [dateFormatter dateFromString:dateString];
//    NSTimeInterval timeInMiliseconds = [dateFromString timeIntervalSince1970]*1000.0;
    long long timeInSeconds = (long long)[dateFromString timeIntervalSince1970]*1.0;
    //long long milliseconds = (long long)([dateFromString timeIntervalSince1970] * 1000.0);
//    long long milliseconds = timeInSeconds * 1000.0;
    
    //NSLog(@"---->timeInMiliseconds=%f && round=%f && timeInSeconds=%lld && milliseconds=%lld", timeInMiliseconds, round(timeInMiliseconds), timeInSeconds, milliseconds);
    return timeInSeconds;
}
